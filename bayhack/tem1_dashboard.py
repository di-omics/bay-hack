...
def replay_tem1_receipt(path: str | Path) -> dict:
    ...
    transition = payload.get("round_transition")
    if (
        not isinstance(transition, dict)
        or transition.get("loop_closed") is not True
        or not verify_receipt_integrity(transition)
    ):
        raise TEM1ReceiptError(
            "TEM-1 receipt is missing a valid round transition proof"
        )
    payload = dict(payload)
    payload["source_mode"] = payload.get("mode", "unknown")
...
.proofline{display:grid;grid-template-columns:1fr auto 1fr;gap:12px;align-items:center;border:1px solid var(--line);border-radius:13px;padding:13px;margin:9px 0}.proofline .arrow{color:var(--matcha);font-size:24px;font-weight:800}.proofline b{color:var(--deep)}.proofline span{display:block;color:var(--muted);font:500 10px var(--mono);margin-top:3px}.proofsummary{border-left:4px solid var(--matcha);background:var(--wash);padding:13px 15px;margin-bottom:12px}.proofsummary b{color:var(--deep)}
...
<div class="grid2"><section class="card"><h3>Round 1 screen plate</h3><p class="note">Broad candidate coverage with controls at the plate edges.</p><div class="plate" id="round1plate"></div></section>
<section class="card"><h3>Round 2 evidence-driven plate</h3><p class="note">Only compounds selected from accepted round 1 measurements return across multiple doses.</p><div class="plate" id="round2plate"></div></section></div>
<div class="grid2" style="margin-top:18px"><section class="card"><h3>Round 1 ranking</h3><p class="note">Selection score is inhibition minus one standard error.</p><div id="ranking"></div></section>
<section class="card"><h3>Round 1 data changed the Round 2 plate</h3><p class="note">A sealed transition proof links measured evidence to every advanced compound, dose, and well.</p><div id="transition"></div></section></div>
...
function transitionProof(t){const p=t.plate_change;let html=`<div class="proofsummary"><b>${p.round1_compounds_tested} tested &rarr; ${p.round2_compounds_advanced} advanced &rarr; ${p.round2_concentration_conditions} dose conditions</b><div class="note" style="margin:4px 0 0">QC pass, model update, ranked selection, and plate verification are all sealed.</div></div>`;for(const row of t.decisions.advanced){html+=`<div class="proofline"><div><b>${row.compound_id}</b><span>${fmt(row.round1_mean_inhibition_pct,1)}% &plusmn; ${fmt(row.round1_standard_error_pct,1)}% in round 1</span></div><div class="arrow">&rarr;</div><div><b>${row.round2_dose_factors.map(v=>v+'x').join(' · ')}</b><span>${row.round2_wells.length} round 2 wells</span></div></div>`}return html}
...
function render(d){$('#output').classList.remove('hidden');pipeline(true);const e=d.protein_synthesis.confirmation,r1=d.rounds[0],r2=d.rounds[1],t=d.round_transition,f=d.follow_up;const replay=d.mode==='receipt-replay';$('#status').textContent=replay?`safe receipt replay · ${d.source_mode} · zero hardware commands`:'simulation complete · physical protocol still locked';$('#banner').innerHTML=`<div class="banner"><h2>TEM-1 expression confirmed before screening</h2><p>${fmt(e.fold_over_background,2)}x over no-template control &middot; expression CV ${fmt(e.expression_cv_pct,2)}% &middot; <span class="prov">${e.provenance.toUpperCase()}</span></p></div>`;$('#stats').innerHTML=`<div class="stat"><b>${fmt(e.fold_over_background,1)}x</b><span>expression / background</span></div><div class="stat"><b>${fmt(r1.assay_qc.z_prime,3)}</b><span>round 1 Z-prime</span></div><div class="stat"><b>${r2.plan.selection_rationale.selected.length}</b><span>compounds advanced</span></div><div class="stat"><b>${fmt(f.inhibition_50_factor_estimate,2)}x</b><span>relative I50 estimate</span></div><div class="stat"><b>${fmt(f.mean_inhibition_pct,1)}%</b><span>best inhibition</span></div>`;$('#round1plate').innerHTML=wellMap(r1);$('#round2plate').innerHTML=wellMap(r2);$('#ranking').innerHTML=ranking(r1);$('#transition').innerHTML=transitionProof(t);$('#dose').innerHTML=dose(r2);$('#final').innerHTML=`<div><span class="pill">FOLLOW-UP EXECUTED</span><br><strong>${f.compound_id} at ${f.concentration_factor}x nominated</strong><div class="note" style="margin:6px 0 0">${fmt(f.mean_inhibition_pct,2)}% ${f.provenance} inhibition &middot; relative I50 ${fmt(f.inhibition_50_factor_estimate,2)}x &middot; monotonic curve ${f.dose_response_monotonic?'PASS':'REVIEW'}</div></div><div class="pill">${d.mode.toUpperCase()}</div>`}
async function proveRefusal(){const r=await fetch('/api/refusal');const d=await r.json();$('#output').classList.remove('hidden');pipeline(false);$('#status').textContent='screen refused before compound execution';$('#banner').innerHTML=`<div class="banner bad"><h2>REFUSED: TEM-1 expression not confirmed</h2><p>${d.confirmation.reasons.join('; ')}. Compound wells read: ${d.compound_screen.wells_read}. Model updated: ${d.world_model.updated}. Round 2 planned: ${d.round2.planned}.</p></div>`;$('#stats').innerHTML=`<div class="stat"><b>${fmt(d.confirmation.fold_over_background||1,2)}x</b><span>expression / background</span></div><div class="stat"><b>0</b><span>compound wells read</span></div><div class="stat"><b>0</b><span>model updates</span></div><div class="stat"><b>0</b><span>round 2 plans</span></div><div class="stat"><b>0</b><span>commands after failure</span></div>`;$('#round1plate').innerHTML='';$('#round2plate').innerHTML='';$('#ranking').innerHTML='<p class="note">No compound result exists because the upstream biological gate failed.</p>';$('#transition').innerHTML='<div class="proofsummary"><b>No transition proof exists</b><div class="note" style="margin:4px 0 0">The upstream biological gate failed.</div></div>';$('#dose').innerHTML='<div class="head">round 2 blocked</div>';$('#final').innerHTML='<div><strong>Recovery: inspect protein synthesis controls</strong><div class="note">Do not turn background noise into an inhibitor claim.</div></div>'}
</script></body></html>"""
