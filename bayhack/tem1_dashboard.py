...
.well.activity{background:#dce8fb;border-color:#9ebbe9;color:#285ca7}.well.inhibited{background:#f7e6ba;border-color:#dfbd68;color:#8b6116}.well.blank{background:#f2f2f2;border-color:#cfcfcf}.well.candidate{color:#fff;border-color:#fff}
...
function wellMap(round){const values={}; round.candidates.forEach(c=>c.wells.forEach(w=>values[w]=c.mean_inhibition_pct));const roles={};round.plan.assignments.forEach(a=>roles[a.well]=a.role);const rows='ABCDEFGH';let html='';for(const row of rows){for(let c=1;c<=12;c++){const w=row+c,role=roles[w]||'',v=values[w];let cls='well';let style='';if(role==='activity_control')cls+=' activity';else if(role==='inhibition_control')cls+=' inhibited';else if(role==='blank')cls+=' blank';else if(role==='candidate'){cls+=' candidate';const light=82-Math.max(0,Math.min(100,v))*.34;style=`background:hsl(128 34% ${light}%);`}html+=`<div class="${cls}" style="${style}" title="${w} ${role} ${v===undefined?'':fmt(v)+'% inhibition'}">${w}</div>`}}return html}
